...
for pair_idx, (dataset1_name, dataset2_name) in enumerate(dataset_pairs, 1):
    ...
    # ============================================
    # Export Error Analysis Layers to GeoPackage
    # ============================================
    
    print(f"\n{'='*60}")
    print(f"CREATING ERROR ANALYSIS LAYERS")
    print(f"{'='*60}")
    
    # Layer 1: Correct matches (GREEN)
    print("\nCreating correct matches layer...")
    
    correct_features = []
    for g1_id in tqdm(correct_matches, desc="Correct matches"):
        # Find the match in predicted results
        match = next((m for m in predicted_matches if m['g1_id'] == g1_id), None)
        if match:
            line = LineString([
    ...
    gdf_correct = gpd.GeoDataFrame(correct_features, crs='EPSG:2154')
    
    # Layer 2: Incorrect matches (RED)
    print("Creating incorrect matches layer...")
    
    incorrect_features = []
    for error in tqdm(incorrect_matches, desc="Incorrect matches"):
        g1_id = error['g1_id']
        pred_g2 = error['predicted_g2']
        true_g2 = error['true_g2']
        error_dist = error['error_distance']
        
        # Get coordinates
        x_g1 = Graph_1.nodes[g1_id]['x']
        y_g1 = Graph_1.nodes[g1_id]['y']
        x_pred_g2 = Graph_2.nodes[pred_g2]['x']
        y_pred_g2 = Graph_2.nodes[pred_g2]['y']
        
        # Find similarities from predicted results
        match = next((m for m in predicted_matches if m['g1_id'] == g1_id), None)
        
        line = LineString([(x_g1, y_g1), (x_pred_g2, y_pred_g2)])
        
        incorrect_features.append({
            'g1_id': g1_id,
            'predicted_g2': pred_g2,
            'true_g2': true_g2,
            'error_dist_m': error_dist,
            'embedding_sim': match['embedding_sim'] if match else None,
            'spatial_sim': match['spatial_sim'] if match else None,
            'combined_sim': match['combined_sim'] if match else None,
            'status': 'incorrect',
            'geometry': line
        })
    
    gdf_incorrect = gpd.GeoDataFrame(incorrect_features, crs='EPSG:2154')
    
    # Layer 3: Missed anchors (ORANGE POINTS)
    print("Creating missed anchors layer...")
    
    missed_features = []
    for g1_id in tqdm(missed_anchors, desc="Missed anchors"):
        x_g1 = Graph_1.nodes[g1_id]['x']
        y_g1 = Graph_1.nodes[g1_id]['y']
        
        # Get true G2 match from ground truth
        true_g2 = ground_truth_anchors[g1_id]['g2_id']
        
        point = Point(x_g1, y_g1)
        
        missed_features.append({
            'g1_id': g1_id,
            'true_g2': true_g2,
            'degree_g1': Graph_1.degree[g1_id],
            'status': 'missed',
            'geometry': point
        })
    
    # Handle empty or non-empty missed anchors
    if len(missed_features) > 0:
        gdf_missed = gpd.GeoDataFrame(missed_features, crs='EPSG:2154')
    else:
        from shapely.geometry import Point
        gdf_missed = gpd.GeoDataFrame({
            'g1_id': [],
            'true_g2': [],
            'degree_g1': [],
            'status': [],
    ...
    # Layer 4: Ground truth anchors (BLUE) - sample for performance
    print("Creating ground truth anchors layer (sample)...")
    
    sample_size = min(1000, len(ground_truth_anchors))
    sampled_anchors = dict(list(ground_truth_anchors.items())[:sample_size])
    
    gt_features = []
    for g1_id, gt_data in tqdm(sampled_anchors.items(), desc="Ground truth sample"):
        g2_id = gt_data['g2_id']
        
        x_g1 = Graph_1.nodes[g1_id]['x']
        y_g1 = Graph_1.nodes[g1_id]['y']
        x_g2 = Graph_2.nodes[g2_id]['x']
        y_g2 = Graph_2.nodes[g2_id]['y']
        
        line = LineString([(x_g1, y_g1), (x_g2, y_g2)])
        
        gt_features.append({
            'g1_id': g1_id,
            'g2_id': g2_id,
            'distance_m': gt_data['distance'],
            'status': 'ground_truth',
            'geometry': line
        })
    
    gdf_gt = gpd.GeoDataFrame(gt_features, crs='EPSG:2154')
    
    # Save all layers to GeoPackage
    print(f"\nSaving to GeoPackage...")
    
    gpkg_filename = f"alignment_error_analysis_{method}_{dataset1_name}_{dataset2_name}.gpkg"
    gpkg_path = OUTPUTS_DIR / gpkg_filename
    
    gdf_correct.to_file(gpkg_path, layer='correct_matches', driver='GPKG')
    print(f"  ✓ Layer 'correct_matches': {len(gdf_correct):,} features (GREEN)")
    
    if len(gdf_incorrect) > 0:
        gdf_incorrect.to_file(gpkg_path, layer='incorrect_matches', driver='GPKG', mode='a')
        print(f"  ✓ Layer 'incorrect_matches': {len(gdf_incorrect):,} features (RED)")
    
    if len(gdf_missed) > 0:
        gdf_missed.to_file(gpkg_path, layer='missed_anchors', driver='GPKG', mode='a')
        print(f"  ✓ Layer 'missed_anchors': {len(gdf_missed):,} features (ORANGE POINTS)")
    
    gdf_gt.to_file(gpkg_path, layer='ground_truth_sample', driver='GPKG', mode='a')
    print(f"  ✓ Layer 'ground_truth_sample': {len(gdf_gt):,} features (BLUE)")
    ...
    print(f"\n{'='*60}")
    print(f"QGIS VISUALIZATION GUIDE")
    print(f"{'='*60}")
    print(f"""
Load in QGIS: {gpkg_path}

Layers and suggested styling:

1. correct_matches ({len(gdf_correct):,} lines)
   - Color: Green (#00FF00)
   - Width: 1
   - Opacity: 30%
   - Shows successful alignments

2. incorrect_matches ({len(gdf_incorrect):,} lines)
   - Color: Red (#FF0000)
   - Width: 3
   - Opacity: 100%
   - Shows errors - zoom in to investigate!

3. missed_anchors ({len(gdf_missed):,} points)
   - Color: Orange (#FF8800)
   - Size: 8
   - Symbol: Circle
   - Shows ground truth anchors not found

4. ground_truth_sample ({len(gdf_gt):,} lines sample)
   - Color: Blue (#0000FF)
   - Width: 1
   - Opacity: 20%
   - Reference layer
...
Analysis tips:
- Turn off 'correct_matches' to focus on errors
- Zoom to 'incorrect_matches' to see local context
- Check if errors are near junctions, boundaries, or dense regions
- Inspect 'missed_anchors' attributes
""")
